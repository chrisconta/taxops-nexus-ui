...
export const ChatWindow = () => {
  ...
  return <div className="flex flex-col h-full max-h-[600px]">
      {/* Chat Messages */}
        ...
        {messages.length === 0 ? <div className="flex items-center justify-center h-full">
            ...
            {messages.map(message => <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-full ${message.role === 'user' ? 'max-w-xs lg:max-w-md' : 'max-w-4xl'}`}>
                   ...
                   {message.requiresData && !message.dataCollected && !dataCollectors.has(message.id) && <TransactionDataCollector messageId={message.id} missingParams={message.missingParams} onDataSubmitted={() => {
      ...
      <div className="flex-shrink-0 p-6 border-t border-glass-border bg-glass-bg/30">
        <div className="flex space-x-3">
          <Input value={input} onChange={e => setInput(e.target.value)} placeholder="Type your message... (max 4000 chars)" className="flex-1 bg-glass-bg/20 border-glass-border text-white placeholder:text-taxops-gray-light" onKeyPress={handleKeyPress} disabled={isLoading} maxLength={4000} />
          <Button onClick={handleSend} disabled={!input.trim() || isLoading || input.length > 4000} className="bg-primary hover:bg-primary/80">
